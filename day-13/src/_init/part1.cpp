...
Because the vertical line is not perfectly in the middle of the pattern, part of the pattern (column 1) has nowhere to reflect onto and can be ignored;
every other column has a reflected column within the pattern and must match exactly: column 2 matches column 9, column 3 matches 8, 4 matches 7, and 5 matches 6.
...
int part1(std::string input_file_path)
{
  ...
  std::vector<std::vector<std::string>> maps = subdivide_maps(document);
  std::vector<int> results;
  for (std::vector<std::string> map : maps)
  {
    if (map.size() == 0)
      continue;
    std::vector<Complex> complex_map = make_complex(map);
    results.push_back(solve(complex_map, map[0].size() - 1, map.size() - 1));
  }
