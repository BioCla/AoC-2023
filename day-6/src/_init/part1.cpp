...
The organizer explains that it's not really a traditional race
    - instead, you will get a fixed amount of time during which your boat has to travel as far as it can, and you win if your boat goes the farthest.
...
int part1(std::string input_file_path)
{
    ...
    std::vector<std::string> times_str_split = split(document[0].substr(document[0].find(": ") + 2), " ");
    std::vector<std::string> distances_str_split = split(document[1].substr(document[1].find(": ") + 2), " ");

    std::vector<int> times;
    std::vector<int> distances;

    for (std::string time_str : times_str_split)
        if (!time_str.empty())
            times.push_back(std::stoi(time_str));

    for (std::string distance_str : distances_str_split)
        if (!distance_str.empty())
            distances.push_back(std::stoi(distance_str));

    int total_ways_to_win = 1;
    ...
    for (size_t i = 0; i < times.size(); i++)
    {
        ...
        {
            int speed = j + 1;
            int time_left = time - speed;
            int distance_traveled = speed * time_left;
