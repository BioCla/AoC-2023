...
int part1(std::string input_file_path)
{
  ...
  while (!document_file.eof())
  {
    ...
    for (size_t i = 0; i < line.size(); i++)
    {
      ...
      {
        start_position.x = i;
        start_position.y = map.size();
      }
      line_vector.push_back(line[i]);
    }
    map.push_back(line_vector);
  }
  document_file.close();

  std::vector<Position> positions_to_check;
  positions_to_check.push_back(start_position);
  std::unordered_map<std::string, int> positions_checked;
  positions_checked[std::to_string(start_position.x) + "," + std::to_string(start_position.y)] = 0;
  int max_distance = 0;
  while (positions_to_check.size() > 0)
  {
    Position current_position = positions_to_check[0];
    positions_to_check.erase(positions_to_check.begin());
    int current_distance = positions_checked[std::to_string(current_position.x) + "," + std::to_string(current_position.y)];
    if (current_distance > max_distance)
      max_distance = current_distance;
    std::vector<Position> possible_directions = get_possible_directions_from_current_position(map, current_position);
    for (auto direction : possible_directions)
    {
      Position new_position = current_position + direction;
      std::string new_position_string = std::to_string(new_position.x) + "," + std::to_string(new_position.y);
      if (positions_checked.find(new_position_string) == positions_checked.end())
      {
        positions_to_check.push_back(new_position);
        positions_checked[new_position_string] = current_distance + 1;
      }
    }
  }
  return max_distance;
