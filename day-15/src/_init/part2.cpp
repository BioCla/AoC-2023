...
The book goes on to describe a series of 256 boxes numbered 0 through 255. The boxes are arranged in a
line starting from the point where light enters the facility. The boxes have holes that allow light to
...
Inside each box, there are several lens slots that will keep a lens correctly positioned to focus light
passing through the box. The side of each box has a panel that opens to allow you to insert or remove lenses as necessary.
...
it is present in the box. Then, move any remaining lenses as far forward in the box as they can go without changing their order,
...
of the lens that needs to go into the relevant box; be sure to use the label maker to mark the lens with the label given in the
...
int part2(std::string input_file_path)
{
    ...
    std::vector<std::string> labels = split(document[0], ",");
    std::unordered_map<u_int, std::vector<std::pair<std::string, int>>> boxes;
    for (std::string label : labels)
    {
        std::vector<std::string> parts = split(label, label.find('=') != std::string::npos ? "=" : "-");
        std::string instruction = parts[0]; int focal_length = parts[1] == "" ? -1 : std::stoi(parts[1]);
        if (label.find('=') != std::string::npos)
        {
            if (std::find_if(boxes[hash(instruction)].begin(), boxes[hash(instruction)].end(), [instruction](std::pair<std::string, int> const& pair) { return pair.first == instruction; }) != boxes[hash(instruction)].end())
                boxes[hash(instruction)][std::find_if(boxes[hash(instruction)].begin(), boxes[hash(instruction)].end(), [instruction](std::pair<std::string, int> const& pair) { return pair.first == instruction; }) - boxes[hash(instruction)].begin()].second = focal_length;
            else
                boxes[hash(instruction)].push_back(std::make_pair(instruction, focal_length));
        }
        else
            boxes[hash(instruction)].erase(std::remove_if(boxes[hash(instruction)].begin(), boxes[hash(instruction)].end(), [instruction](std::pair<std::string, int> const& pair) { return pair.first == instruction; }), boxes[hash(instruction)].end());
    }
