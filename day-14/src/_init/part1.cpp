...
int part1(std::string input_file_path)
{
    ...
    rocks_layout = roll_north(rocks_layout);

    int load_on_north_support_beams = 0;
    for (size_t i = rocks_layout.size() - 1; i < rocks_layout.size(); i--)
        for (size_t j = 0; j < rocks_layout[i].size(); j++)
            if (rocks_layout[i][j] == 'O')
                load_on_north_support_beams += rocks_layout.size() - i;

    return load_on_north_support_beams;
}
