...
Once all of the originals and copies have been processed,
    you end up with 1 instance of card 1, 2 instances of card 2, 4 instances of card 3, 8 instances of card 4, 14 instances of card 5, and 1 instance of card 6.
...
int part2(std::string input_file_path)
{
    ...
    {
        std::string *parsed_line = parse_line(line);
        std::vector<std::string> owned_numbers = split(parsed_line[1], " ");
        int winning_numbers_count = 0;
        for (const auto &owned_number : owned_numbers)
        {
            bool is_number = owned_number != "" && owned_number != " ";
            if (parsed_line[0].find(" " + owned_number + " ") != std::string::npos && is_number)
                winning_numbers_count++;
        }
        matches.push_back(winning_numbers_count);
        delete[] parsed_line;
    }

    std::vector<int> cards(matches.size(), 1);
