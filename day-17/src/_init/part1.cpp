...
best way to get the crucible from the lava pool to the machine parts factory.
...
temperature, and hundreds of other parameters to calculate exactly how much heat loss can be expected for a crucible entering any particular city block.
...
int part1(std::string input_file_path)
{
    ...
    return minimum_heat_dispersal(1, 3, grid);
}